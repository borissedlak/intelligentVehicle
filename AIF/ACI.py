...
class ACI:
    ...
    def __init__(self, device_name, show_img=False, load_model=None, distance_slo=40, network_slo=(420 * 30 * 10)):
        ...
        if load_model:
            ...
            utils.export_BN_to_graph(self.model, vis_ls=['circo'], save=True, name="raw_model", show=self.show_img)
            self.foster_bn_retrain = 0.2
            self.backup_data = utils.prepare_samples(pd.read_csv(f"backup_entire_data_{device_name}.csv"),
        ...
        self.ig_matrix = utils.interpolate_values(self.ig_matrix)
        self.ig_matrix[0][0], self.ig_matrix[5][4], self.ig_matrix[5][0], self.ig_matrix[0][4], \
    ...
    def iterate(self, c_stream_count):
        ...
        s = -1
        if utils.verify_all_slo_parameters_known(self.model, self.current_batch):
            s = utils.get_surprise_for_data(self.model, self.current_batch)
    ...
    def get_best_configuration(self):
        pv_interpolated = utils.interpolate_values(self.pv_matrix)
        ra_interpolated = utils.interpolate_values(self.ra_matrix)
        ig_interpolated = utils.interpolate_values(self.ig_matrix)
    ...
    def calculate_factors(self, model, c_pixel, c_fps, c_stream_count):
        ...
        inference = VariableElimination(
            utils.get_mbs_as_bn(model, ["success", "in_time", "bitrate", "stream_count", "network", "distance"]))
        ...
        for br in bitrate_list:
            ...
            try:
                ...
                ra = utils.get_true(inference.query(variables=["in_time", "network"], evidence=evidence))
        ...
        if len(unknown_combinations) > 0:
            ...
            for p, f, s in unknown_combinations:
                ...
                pv_predict = utils.cap_0_1(self.stream_regression_model_pv.predict(input_vector)[0])
                self.pv_matrix[ACI.pixel_list.index(p)][ACI.fps_list.index(f)] = pv_predict
                ra_predict = utils.cap_0_1(self.stream_regression_model_ra.predict(input_vector)[0])
    ...
    def retrain_parameter(self, full_retrain=False):
        if full_retrain:
            if self.load_model:
                utils.print_in_red("Should not happen when loading model")
    ...
    def bnl(self, samples):
        ...
        utils.export_BN_to_graph(dag, vis_ls=['circo'], save=True, name="raw_model", show=self.show_img)
    ...
    def load_last_batch(self):
        ...
        samples = utils.prepare_samples(samples, self.c_distance_bar, self.c_network_bar)
